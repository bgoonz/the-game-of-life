var Grid = (function () {
  function getPosition(element) {
    ...
    do {
      ...
      typeof element.offsetLeft !== "undefined"
    );
  ...
  function Grid(placeholder, cell_size) {
    ...
    if (typeof placeholder.getContext !== "function") {
      ...
      this.hover_canvas.style.position =
        this.background_canvas.style.position =
        this.cells_canvas.style.position =
          "absolute";
      this.hover_canvas.style.top =
        this.background_canvas.style.top =
        this.cells_canvas.style.top =
          0;
      this.hover_canvas.style.left =
        this.background_canvas.style.left =
        this.cells_canvas.style.left =
          0;
      this.hover_canvas.width =
        this.background_canvas.width =
        this.cells_canvas.width =
          this.width;
      this.hover_canvas.height =
        this.background_canvas.height =
        this.cells_canvas.height =
          this.height;
      ...
      this.bg_ctx =
        this.h_ctx =
        this.c_ctx =
          this.hover_canvas.getContext("2d");
  ...
  Grid.prototype.init = function () {
    ...
    this.hover_canvas.onmousemove = function (e) {
      if (down) {
        if (draw) {
          self[mark ? "markCell" : "clearCell"](
            self.getAbsoluteCellFromPosition(e)
          );
          changed = true;
  ...
  Grid.prototype.isMarked = function (cell) {
    return !!(
      this.markedCells_[cell[0]] && this.markedCells_[cell[0]][cell[1]]
    );
  };

  Grid.prototype.markCell = function (cell) {
    (this.markedCells_[cell[0]] || (this.markedCells_[cell[0]] = {}))[
      cell[1]
    ] = true;
  };
  ...
  Grid.prototype.draw = function (force) {
    ...
    if (
      this.hover_cell &&
      this.hoverIndicator &&
      (!this.prev_hover_cell ||
        this.hover_cell.toString() !== this.prev_hover_cell.toString())
    ) {
  ...
  Grid.prototype.drawBasicGrid = function (clear) {
    ...
    for (
      var w = -(this.offsetX % this.cell_size);
      w < this.width;
      w += this.cell_size
    ) {
    ...
    for (
      var h = -(this.offsetY % this.cell_size);
      h < this.height;
      h += this.cell_size
    ) {
  ...
  Grid.prototype.drawHover = function (clear) {
    ...
    var x =
        -(this.offsetX % this.cell_size) + this.hover_cell[0] * this.cell_size,
      y =
        -(this.offsetY % this.cell_size) + this.hover_cell[1] * this.cell_size;
    this.h_ctx.save();
